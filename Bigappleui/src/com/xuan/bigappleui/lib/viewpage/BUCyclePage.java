 ...
 * @author xuan
 */
public class BUCyclePage extends ViewGroup {
	private static final int TOUCH_STATE_REST = 0;// 空闲状态
	private static final int TOUCH_STATE_SCROLLING = 1;// 正在滚动状态
	private int touchState = TOUCH_STATE_REST;

	private final OnScrollCompleteListener internalOnScrollCompleteListener;
	...
	public BUCyclePage(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public BUCyclePage(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs);
		...
		internalOnScrollCompleteListener = new OnScrollCompleteListener() {
			...
			public void onScrollComplete(int toPosition) {
				if (toPosition > 0 && toPosition < mAdapter.getCount() + 1) {
	...
	public void setAdapter(BaseAdapter adapter) {
		...
		addView(adapter.getView(count - 1, null, null));
		for (int i = 0; i < count; i++) {
			addView(adapter.getView(i, null, null));
		}
		addView(adapter.getView(0, null, null));
	}
