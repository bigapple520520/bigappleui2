...
public class FileExplorerActivity extends Activity {
	...
	/** 选中的文件列表 */
	private final ArrayList<String> selectedFilePathList = new ArrayList<String>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 获取参数
		ifMultiple = getIntent()
				.getBooleanExtra(PARAM_IF_MULTIPLE_CHOICE, true);
		if (!ifMultiple) {
		...
		fileInfoListAdapter = new FileInfoListAdapter(this, fileInfoList,
				selectedFilePathList, new SelectImageViewOnClickListener() {
					@Override
					public void onClick(ImageView imageView, FileInfo fileInfo) {
						if (ifMultiple) {
							// 多选
							if (selectedFilePathList
									.contains(fileInfo.filePath)) {
								selectedFilePathList.remove(fileInfo.filePath);
								imageView.setImageDrawable(DrawableHelper
										.getCheckBoxNormalIcon());
							} else {
								if (-1 != limitCount
										&& selectedFilePathList.size() >= limitCount) {
									// 到限制数量了，不让选了
									Toast.makeText(FileExplorerActivity.this,
											"最多只能选择" + limitCount + "个文件",
											Toast.LENGTH_LONG).show();
								} else {
									selectedFilePathList.add(fileInfo.filePath);
									imageView.setImageDrawable(DrawableHelper
											.getCheckBoxSelectedIcon());
								}
							}
		...
		root.fileListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> adapterView, View view,
				...
				FileInfo fileInfo = (FileInfo) fileInfoListAdapter
						.getItem(position);
				if (fileInfo.isDir) {
					// 文件夹进入
	...
	private void refreshNavigation(String defaultPath) {
		...
		root.navigationLayout.addView(FileExplorerUIHepler.getNavigationItem(
				this, "当前位置：", null, null));

		root.navigationLayout.addView(FileExplorerUIHepler.getNavigationItem(
				this, "/root", "/", new OnClickListener() {
					@Override
					public void onClick(View view) {
		...
		if ("/".equals(defaultPath)) {
			...
			for (String item : items) {
				...
				root.navigationLayout.addView(FileExplorerUIHepler
						.getNavigationItem(this, "/" + item,
