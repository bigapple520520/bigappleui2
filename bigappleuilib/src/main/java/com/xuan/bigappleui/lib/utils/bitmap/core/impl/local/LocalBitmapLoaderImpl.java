package com.xuan.bigappleui.lib.utils.bitmap.core.impl.local;
...
import com.xuan.bigappleui.lib.utils.bitmap.core.cache.LruMemoryCache;
...
public class LocalBitmapLoaderImpl implements IBitmapLoader {
	...
	public void display(ImageView imageView, String filePath,
		...
		if (defaultBitmapGlobalConfig.isMemoryCacheEnabled()) {
			...
			if (null != bitmap) {
				BULogUtil.d("yes!!!cache is shot!!!");
				config.getDisplayImageListener().loadCompleted(imageView,
						bitmap, config);
				return;
			} else {
				BULogUtil.d("no!!!cache is miss,i need get bitmap from disk!!!");
			}
		}
