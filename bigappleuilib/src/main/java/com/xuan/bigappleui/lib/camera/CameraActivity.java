 ...
 * 1、拷贝下列资源文件到自己的项目中去
 * xml文件
 * bpui_camera_btn_take_picture.xml
 * bpui_camera_progress.xml
 *
 * 图片
 * bpui_camera_btn_take_picture_normal.png
 * bpui_camera_btn_take_picture_pressed.png
 * bpui_camera_flash_close.png
 * bpui_camera_flash_open.png
 * bpui_camera_progress_ball.png
 * bpui_camera_progress_volumn_bg.9.png
 * bpui_camera_progress_volumn_front.9.png
 * bpui_camera_progress_volumn_primary.9.png
 *
 * 布局:
 * bpui_camera.xml
 * 
 ...
 * 界面:
 * com.xuan.bigappleui.lib.camera.BUCameraActivity
 * 权限：
 ...
 * 
 * 3、然后代码调用：
 * BPUICameraUtils.openCamera(context, REQUEST_CODE, outputPath);
 * 
 * @author xuan
 */
public class CameraActivity extends Activity {
	...
	private void findView() {
		cameraPreview = (CameraPreview) findViewById(M
				.id(this, "cameraPreview"));
	...
	private void initWidgets() {
		...
		// 调节焦距
		jiaojuSeekBar
				.setOnSeekBarChangeListener(new OnBuSeekBarChangeListener() {
					@Override
					public void onProgressChanged(SeekBar vBar, int progress,
							boolean fromUser) {
						try {
							CameraUtils.debug(TAG, "-----------progress:"
									+ progress);
		...
		okTv.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				intent.putExtra(CameraActivity.IMAGET_OUTPUT_PATH,
						imageSavePath);
	...
	private final class MPictureCallback implements PictureCallback {
		...
		public void onPictureTaken(final byte[] data, final Camera camera) {
			// 震动一下手感
			Vibrator vibrator = (Vibrator) CameraActivity.this
