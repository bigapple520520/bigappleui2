...
public class AlbumHelper {
	...
	private final Map<String, ImageBucket> bucketMap = new HashMap<String, ImageBucket>();
		...
		if (null == instance) {
			if (context instanceof Activity) {
				...
				throw new IllegalArgumentException(
						"初始化init传入Context类型必须是Activity或者Application");
			}
		}
	}
	 ...
	 * @return
	 */
	public Map<String, ImageBucket> getImagesBucketMapSortByDatemodify(
			boolean needRefresh) {
		if (needRefresh || (!needRefresh && !hasBuildImagesBucketList)) {
			bucketMap.clear();
			buildImagesBucketList();
				...
				Collections.sort(imageBucket.imageList,
						new Comparator<ImageItem>() {
							...
							public int compare(ImageItem imageItem1,
								...
								long dateModifyLong1 = Long
										.valueOf(imageItem1.dateModified);
								long dateModifyLong2 = Long
										.valueOf(imageItem2.dateModified);

								// 注意：这个JDK7里面TimSort排序的标准Comparator写法
								return dateModifyLong1 == dateModifyLong2 ? 0
										: (dateModifyLong1 < dateModifyLong2 ? 1
												: -1);
							}
						});
			}
	...
	public HashMap<String, String> getImageId2ThumbnailPathMap() {
		...
		try {
			if (cursor.moveToFirst()) {
				...
				int imageIdColumnIndex = cursor
						.getColumnIndex(Thumbnails.IMAGE_ID);// 缩略图对应的原图编号
				int thumbnailPathColumnIndex = cursor
						.getColumnIndex(Thumbnails.DATA);// 缩略图的路径
				do {
					imageId = cursor.getInt(imageIdColumnIndex);
					thumbnailPath = cursor.getString(thumbnailPathColumnIndex);
					imageId2ThumbnailPathMap.put(String.valueOf(imageId),
							thumbnailPath);
	...
	private void buildImagesBucketList() {
		...
		String columns[] = new String[] { Media._ID, Media.BUCKET_ID,
				Media.DATA, Media.BUCKET_DISPLAY_NAME, Media.DATE_ADDED,
				Media.DATE_MODIFIED };
		Cursor cursor = context.getContentResolver().query(
		...
		try {
			if (cursor.moveToFirst()) {
				...
				int bucketIdColumn = cursor
						.getColumnIndexOrThrow(Media.BUCKET_ID);// 对应文件夹的编号
				int bucketDisplayNameColumn = cursor
						.getColumnIndexOrThrow(Media.BUCKET_DISPLAY_NAME);// 文件夹的名称
				int imagePathColumn = cursor.getColumnIndexOrThrow(Media.DATA);// 原图的路径
				int dateAddedColumn = cursor
						.getColumnIndexOrThrow(Media.DATE_ADDED);
				...
				do {
					...
					imageItem.thumbnailPath = imageId2ThumbnailPathMap
							.get(imageId);
