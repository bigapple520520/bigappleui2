...
import com.xuan.bigappleui.lib.pullrefresh.core.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (!isPullUpRefreshEnabled() && !isPullDownRefreshEnabled()) {
			return false;
		}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (absDiff > mTouchSlop || isPullRefreshing() || isPullLoading()) {
				...
				if (isPullDownRefreshEnabled() && isReadyForPullDown()) {
				...
				} else if (isPullUpRefreshEnabled() && isReadyForPullUp()) {
					// 原理如上
					mIsHandledTouchEvent = (Math.abs(getScrollYValue()) > 0 || deltaY < -0.5f);
				}
			}
	 ...
	 */
	protected AbstractLoadingLayout createHeaderLoadingLayout(Context context,
			AttributeSet attrs) {
	 ...
	 * @return LoadingLayout对象
	 */
	protected AbstractLoadingLayout createFooterLoadingLayout(Context context,
			AttributeSet attrs) {
	...
	private void refreshLoadingViewsSize() {
		mHeaderHeight = (null != mHeaderLayout) ? mHeaderLayout
				.getMeasuredHeight() : 0;
		mFooterHeight = (null != mFooterLayout) ? mFooterLayout
				.getMeasuredHeight() : 0;
		setPadding(getPaddingLeft(), getPaddingTop() - mHeaderHeight,
				getPaddingRight(), getPaddingBottom() - mFooterHeight);
	}
	...
	protected void addRefreshableView(Context context, T refreshableView) {
		...
		addView(mRefreshableViewWrapper, new LinearLayout.LayoutParams(
	...
	protected void pullHeaderLayout(float delta) {
		...
		if (isPullDownRefreshEnabled() && !isPullRefreshing()) {
	...
	protected void pullFooterLayout(float delta) {
		...
		int scrollY = Math.abs(getScrollYValue());
		if (isPullUpRefreshEnabled() && !isPullLoading()) {
	...
	public void setPullDownRefreshEnabled(boolean pullDownRefreshEnabled) {
		mPullDownRefreshEnabled = pullDownRefreshEnabled;
	}

	@Override
	public void setPullUpRefreshEnabled(boolean pullUpRefreshEnabled) {
		mPullUpRefreshEnabled = pullUpRefreshEnabled;
	}

	@Override
	public void setScrollUpRefreshEnabled(boolean scrollUpRefreshEnabled) {
		mScrollUpRefreshEnabled = scrollUpRefreshEnabled;
	}
	...
	@Override
	public boolean isScrollUpRefreshEnabled() {
		return mScrollUpRefreshEnabled;
	}
