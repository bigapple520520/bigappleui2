package com.xuan.bigappleui.lib.view.tab.sw;
 ...
 */
public class BUSwTabHost extends LinearLayout {
	/** 当前位置 */
	private int currentPosition = -1;

	/** tab的View数组 */
	private final List<View> tabList = new ArrayList<View>();
	/** tab内容页View数组 */
	private final List<View> tabContentList = new ArrayList<View>();

	/** tabHost */
	private BUITabHost tabHost;
	...
	/** 内容可以进行滑动切换 */
	private BUHackyViewPager viewPage;
	 ...
	 * @param context
	 */
	public BUSwTabHost(Context context) {
		super(context);
		init(context);
	}
	 ...
	 * @param attrs
	 */
	public BUSwTabHost(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	 ...
	 * @param defStyle
	 */
	public BUSwTabHost(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (null != indicator) {
			FrameLayout.LayoutParams indicatorLp = (FrameLayout.LayoutParams) indicator
					.getLayoutParams();
	 ...
	 */
	public void setOnPositionChangeListener(
	 ...
	 *            内容的View列表
	 */
	public void addTabsAndContents(int index, List<View> tabList,
	 ...
	 * @param tabHost
	 */
	public void setTabHost(BUITabHost tabHost) {
	...
	private void setupTabs() {
		...
		if (-1 != tabsLayoutHeight) {
			...
			if (null == tabsLayoutLp) {
				tabsLayoutLp = new LinearLayout.LayoutParams(
						ViewGroup.LayoutParams.MATCH_PARENT, tabsLayoutHeight);
			} else {
		...
		for (View view : tabList) {
			LinearLayout.LayoutParams viewLp = new LinearLayout.LayoutParams(0,
	...
	private void setupIndicate() {
		...
		if (-1 != indicatorHeight) {
			// 调整高度
			ViewGroup.LayoutParams indicatorLp = indicatorLayout
					.getLayoutParams();
			if (null == indicatorLp) {
	...
	private void setupDividerFromIndicatorToTabContents() {
		View divider = tabHost
				.getDividerFromIndicatorToTabContents(getContext());
		if (null != divider) {
			addView(divider);
		}
	}
	...
	private void setupContents() {
		...
		viewPage = new BUHackyViewPager(getContext());
		...
		viewPage.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
			...
			public void onPageScrolled(int position, float positionOffset,
				...
				int paddingLeft = position * indicatorWidth
						+ positionOffsetPixels / tabList.size();
	...
	private void checkSizeEquals() {
		if (tabList.size() != tabContentList.size()) {
			throw new IllegalArgumentException(
					"Size of tab must equals size of tabContent");
		}
	}
